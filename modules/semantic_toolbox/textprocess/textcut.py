...
"""
stop_words = importlib.resources.read_text('textprocess', 'cn_stopwords.txt').split('\n')
...
def textcut(text,stop=1,stopwords=stop_words):
    ...
    if stop == 1:
        words = [w for w in words_cut if (validate_word(w) and w not in stopwords)]
    else:
        words = [w for w in words_cut if validate_word(w)]
...
def savefile(data,outfile,filetype,index_in=0,header_in=0):
    ...
    if filetype=='.txt':
        ...
        df.to_excel(outfile,index=index_in,header=header_in)
    else:
