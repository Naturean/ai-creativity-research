...
def get_model(model_path):
    if (model_path.endswith('.bin')):
        model = gensim.models.fasttext.load_facebook_vectors(os.path.join(model_path))
    else:
...
def get_sentence_vector(s, model=0, preprocess=0):
    ...
    # vectorized_sentence  = [v for v in vectorized_sentence if isinstance(v, np.ndarray) and not np.any(np.isnan(v))]
