...
def dis_sentences(s1, s2, model=0, preprocess=0, debug=False):
    v1 = get_sentence_vector(s1,model,preprocess)
    ...
    if debug:
        ...
        v1_n = _unitvec(v1)
        v2_n = _unitvec(v2)
        print("v1_n shape:", v1_n.shape)
        print("v2_n shape:", v2_n.shape)
        cosine_sim = np.dot(v1_n, v2_n)
        print("cosine_sim shape:", cosine_sim.shape)

    cosine_sim = np.dot(_unitvec(v1), _unitvec(v2))
    # print(cosine_sim)
    return 1 - max(0, cosine_sim)
